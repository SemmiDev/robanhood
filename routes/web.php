...
function sendOneSignalNotification($oneSignalIds, $heading, $content, $url = null, $additionalData = [])
{
    try {
        // Validate inputs
        ...
        // 'body' => '{"app_id":"YOUR_APP_ID","contents":{"en":"Your message body here."},"included_segments":["Test Users"]}',
        ...
        \Log::info('OneSignal notification sent successfully', [
            'recipients' => count(is_array($oneSignalIds) ? $oneSignalIds : [$oneSignalIds]),
        ...
        // Log error
        \Log::error('Error sending OneSignal notification', [
            'error' => $e->getMessage(),
            'recipients' => $oneSignalIds
        ]);
...
Route::get('/send', function () {
    ...
    return sendOneSignalNotification($id, $title, $body, "https://robanhood-staging.expert.web.id/histori-notifikasi");
});

Route::get('/haversine', function () {
    $latitudeFrom = 0.4715927367772029;
    $longitudeFrom = 101.37951640112456;
    $latitudeTo = 0.4714961805246535;
    $longitudeTo = 101.3795526109452;
...
Route::get('/update-coordinates/{user_id}', function ($userId) {
    $user = User::findOrFail($userId);
    $targetLat = $user->latitude_terakhir;
    $targetLong = $user->longitude_terakhir;

    $currentLat = $user->latitude_terakhir - 0.001; // Start slightly offset
    $currentLong = $user->longitude_terakhir - 0.001;

    // Calculate step size (0.0001 degrees ≈ 11 meters)
    $stepLat = ($targetLat - $currentLat) > 0 ? 0.0001 : -0.0001;
    $stepLong = ($targetLong - $currentLong) > 0 ? 0.0001 : -0.0001;
    ...
    // Update database
    $user->update([
        'latitude_terakhir' => $currentLat,
        'longitude_terakhir' => $currentLong
    ]);
